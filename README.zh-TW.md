...
### 5. 多國語系編輯器 (Localization Editor)

* **統一編輯介面**：提供一個 Webview 編輯器，讓您在單一介面中管理所有 `.arb` 檔案。
* **兩種顯示模式**：
    * **合併視圖 (Combined View)**：將所有語言並排顯示在一個表格中，方便比較。
    * **頁籤視圖 (Tabbed View)**：一次只顯示一種語言的翻譯，簡化編輯流程。
* **進階篩選**：
    * **按鍵或值篩選**：快速找到特定的翻譯內容。
    * **僅顯示 Placeholder**：篩選出包含變數的項目。
* **動態 Placeholder 管理**：
    * 自動偵測 Placeholder (例如 `{name}`)
    * 允許編輯 Placeholder 的類型 (例如 `String`, `num`)
* **自動儲存與熱重載**：儲存時會將所有變更寫回對應的 `.arb` 檔案，並觸發熱重載。
